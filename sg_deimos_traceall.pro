...
;       2008-Oct-15  JDS - changed deimos_slit_match to jds_deimos_slit_match
;       2014-Sep-05  JDS - added slit_match_problem keyword
;
;----------------------------------------------------------------------
pro sg_deimos_traceall, chipno, flatimage, flativar, $
          slitcoords, nslitlets, xpos1, xpos2, ypos1, ypos2, $
          synth1, synth2, indblu, $
          ybin=ybin, plot=plot, pspath=pspath, badmatch=badmatch  , $
                     model=model, slit_match_problem=slit_match_problem
...
; -------- Find some traces - ncoeff=4 means third order fit 
  ...
  if keyword_set(slit_match_problem) then stop
...
;CHANGED CALL TO JDS_DEIMOS_SLIT_MATCH --- 10/15/08
  jds_deimos_slit_match, chipno, tset1, tset2, indblu, $
