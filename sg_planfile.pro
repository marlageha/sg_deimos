...
;   Jun16,  MG creates file for partial datasets  
;----------------------------------------------------------------------
...
pro deimos_plan, s, maskname, rawdatadir
  ...
  if (nflats GE 1) AND (narcs GE 1) AND (nscience GE 1) then begin 
     ...
     printf, wlun, maskname, format='("MASK: ",A)'
     printf, wlun, rawdatadir, format='("RAWDATADIR: ",A)'
     if (nflats ge 1) then printf, wlun, a[flatind].fname, format='("FLATNAME: ",A)'
     if (narcs ge 1) then printf, wlun, a[arcind].fname, format='("ARCNAME: ",A)'
     if (nscience ge 1) then printf, wlun, a[scienceind].fname, format='("SCIENCENAME: ",A)'
...
;  for i=0, ct-1 do s[i] = deimos_parse_header(flist[i])
  for i=0, ct-1 do begin
     print,flist[i]
     s[i] = deimos_parse_header(flist[i])
  endfor
  sind = sort(s.mask)
  s = s[sind]
  ind = uniq(s.mask)
