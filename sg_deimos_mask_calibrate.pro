...
;  2008-Oct-15 - changed deimos_traceall to jds_deimos_traceall - JDS
;  2009-May-4  - added a tweak to the zeropoint of the optical model
;                in the case of bad wavelength solutions; this seems
;                to fix what Cooper says is a recently developed 
;                problem in the optical model for slider 4, but what
;                it will do in other cases has not been tested  - JDS
;  2014-Sep-5  - added wave_offset keyword so the above zeropoint tweak
;                is adjustable (default = 2 A) - JDS
;                added slit_match_problem keyword for
;                jds_deimos_traceall - JDS
; 2016-Jun-10  - Strip away DEEP and legacy code - MG
;----------------------------------------------------------------------
FUNCTION get_anamorph, gratenum, lambda_c

;;; READ-IN THE DEIMOS SPECS FROM FILE.
  READCOL, djs_filepath('deimos_anamorph.dat', ROOT=GETENV('CALIB_DATA')), $
                         grating, lambda, anamor, FORMAT='F,F,F', COMMENT='#'
...
pro sg_deimos_mask_calibrate,  planfile,  chiplist=chiplist, noplot=noplot, $
     slitlist=slitlist, onlyalign=onlyalign, $
...
;  verbset, 4-3*(quick gt 0)
  ...
  if keyword_set(wave_offset) then wave_zeropoint_offset = wave_offset else $
    wave_zeropoint_offset = 2
...
; set the read-noise value.
  readnoise = 2.32       ; ** MG 2015 RN = 2.5, update?
...
; ** MG updated location of file 
  lampfilename = getenv('CALIB_DATA')+linelist ;lamp_NIST.dat'
...
;    if file_test(pixflatname) eq 0 OR quick gt 0 then begin 
	...
	vprint,4,'Using pixflat!'
          ...
          print,'deimos_pixflat_mul[0:*,0:*] = 1'
;          stop
...
; tracing: 7 sec
;CHANGED CALL TO JDS_DEIMOS_TRACEALL --- 10/15/08
      sg_deimos_traceall, chipno, flatinterp, flativar, $
        slitcoords, nslitlets, xpos1, xpos2, ypos1, ypos2, synth1, synth2, $
        indblu, ybin=ybin, plot=plot, pspath=pspath, badmatch=badmatch, $
        model=model_lambda, slit_match_problem=slit_match_problem
...
;ADD WAVELENGTH RANGE FOR EACH SLIT - JDS, 6/5/13
     waverange = fltarr(2, nslitlets)
...
;JDS, 5/04/09
;IF WAVELENGTH SOLUTION FAILS THE FIRST TIME THROUGH, TWEAK THE OPTICAL MODEL SLIGHTLY
;AND TRY IT AGAIN
                iter = iter + 1
                if iter le 3 then begin
;                    stop
                    
;IN CASE THERE IS NOT A FULL MODEL TO START WITH, DO WHAT DEIMOS_WAVEMATCH WAS GOING
;TO DO ANYWAY AND SUBSTITUTE THE TOP/BOTTOM VALUES
                    nomodel = (model_lambda[indblu[slitno]].lambda_y[0] eq 0 $
                               OR model_lambda[indblu[slitno]].lambda_y[1] lt 0) 
                    if nomodel then begin
                        if (model_lambda[indblu[slitno]].lambda_y_bottom[0] NE 0 AND $
                            model_lambda[indblu[slitno]].lambda_y_top[0] NE 0) then $
                          model_lambda[indblu[slitno]].lambda_y = $
                          ( model_lambda[indblu[slitno]].lambda_y_bottom + $
                            model_lambda[indblu[slitno]].lambda_y_top)/2. $
                        else if (model_lambda[indblu[slitno]].lambda_y_bottom[0] NE 0) then $
                          model_lambda[indblu[slitno]].lambda_y = $
                          model_lambda[indblu[slitno]].lambda_y_bottom $
                        else if (model_lambda[indblu[slitno]].lambda_y_top[0] NE 0) then $
                          model_lambda[indblu[slitno]].lambda_y = $
                          model_lambda[indblu[slitno]].lambda_y_top
                    endif    
                    
;THIS WILL PROBABLY CRASH HARD IF THERE IS NO MODEL FOR ANY PART OF THE SLIT, SO STOP:
                    if model_lambda[indblu[slitno]].lambda_y[0] eq 0 then message, $
                      'THIS IS NOT GOING TO WORK - NO OPTICAL MODEL AT ALL FOR THIS SLIT!!!'
                    
;ADD TWEAK TO THE INITIAL WAVELENGTH ZERO POINT (USUALLY THIS GUESS WAS OFF BY ~30
;ANGSTROMS, BUT EVEN A SMALL OFFSET IS ENOUGH TO PUSH THE WAVELENGTH SOLUTION TO THE 
;CORRECT LOCATION) AND THEN RUN THROUGH THE FIT AGAIN
                    model_lambda[indblu[slitno]].lambda_y[0] = $
                      model_lambda[indblu[slitno]].lambda_y[0] + wave_zeropoint_offset
                    vprint,4,'Slit ' + strcompress(string(bluslitno),/remove) + $
                      ': Wavelength solution failed, trying one more time . . .'
                    goto,try_wavefit_again
                endif
...
;ADD WAVELENGTH RANGE FOR EACH SLIT - JDS, 6/5/13
               tempwave = lambda_eval(calibSlit)
               waverange[0,slitno] = min(tempwave)
               waverange[1,slitno] = max(tempwave)
