...
;       2014-Sep-05  JDS - Added tweakmethod keyword and associated options
;       2015-May-11  JDS - Updated setting of tweakmethod
;
;----------------------------------------------------------------------

pro sg_deimos_spslit, chipno, maskno, calibfile, $
          badchip, sciencenames, $
          outdatadir=outdatadir, flat=flat, plot=plot, pixmap=pixmap, $
          quick=quick, tweakmethod=tweakmethod

  readnoise = 2.32
  noisesq = readnoise^2+5.

;SET SKY TWEAK METHOD:
;  1 = JDS SKY TWEAK [DEFAULT]
;  2 = ORIGINAL DEEP2 SKY TWEAK
;  3 = NO SKY TWEAK
  if NOT keyword_set(tweakmethod) then tweak = 1 else tweak = tweakmethod
...
; find global offset (per chip) due to flexure
     ...
     dx = jds_deimos_edge_offset(xpos1, xpos2, ypos1, ypos2, specimage, $
...
;       tweak wavelengths of spslit
        ...
        if quick le 0 then begin
           ...
           case tweak of 
               1: jds_deimos_skytweak, spslit, skytweak=skytweak, /plot
               2: deimos_skytweak, spslit, skytweak=skytweak
               3: skytweak = {medsigma: 0.}
               else: begin
                   print,'Invalid sky tweak method!'
                   stop
                   end
           endcase
...
;if islit ge 4 then stop
;        jds_flag_cr, spslit, flagivar,crmask, medflux=rect_med
;        jds_ldss_crflag,spslit.flux,spslit.ivar,crflag,grow=1,psfsig=6
         jds_ldss_crflag,spslit.flux,spslit.ivar,crflag,psfsig=6
        crmask = spslit.flux*0b       
        if N_elements(crflag) gt 0 then crmask[crflag]=1b         
        flagivar = spslit.ivar*(1b-crmask)
