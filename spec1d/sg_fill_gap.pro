...
function make_ss1d_ovl, spec=spec, ivar=ivar, lambda=lambda, $
                    ssBd=ssBd, ssRd=ssRd, gapsz=gapsz
  npix = 4096
  zero1d = {spec:fltarr(npix), ivar:fltarr(npix), lambda:fltarr(npix), $
            crmask:intarr(npix), bitmask:intarr(npix), $
  ...
  struct_assign, ssBd, ss1
  struct_assign, ssRd, ss2
  crmask = [ss1.crmask,ss2.crmask[gapsz:*]]
  bitmask = [ss1.bitmask,ss2.bitmask[gapsz:*]]
  ormask = [ss1.ormask,ss2.ormask[gapsz:*]]
  infomask = [ss1.infomask,ss2.infomask[gapsz:*]]
  nbadpix = [ss1.nbadpix,ss2.nbadpix[gapsz:*]]
  objpos = mean([ss1.objpos, ss2.objpos])
  fwhm = mean([ss1.fwhm, ss2.fwhm])
  nsigma = mean([ss1.nsigma, ss2.nsigma])
  r1 = mean([ss1.r1, ss2.r1])
  r2 = mean([ss1.r2, ss2.r2])
  skyspec = [ss1.skyspec,ss2.skyspec[gapsz:*]]
  ivarfudge = mean([ss1.ivarfudge, ss2.ivarfudge])
  ss1d = {spec:spec, ivar:ivar, lambda:lambda, $
          crmask:crmask, bitmask:bitmask, ormask:ormask, $
          infomask:infomask, nbadpix:nbadpix, $
...
function sg_fill_gap, file1d, nbuf=nbuf, npoly=npoly, nbins=nbins, $
...
; lastly, fill the gap between the red and blue spectra. 
          ...
          if gapsize lt 0 then begin
             print,'no gap tp fill'              ;  MG-- ADDED CASE WHERE WAVELENGTHS OVERLAP
              ;       OVERLAP REGION IS A WEIGHTED AVERAGE
                 olam = gapsize/dlam
                 lambda1 = ss1.lambda[0:nlam-gapsize]
                 lambda2 = ss2.lambda[olam:*]
...
;               spec = [ss1.spec[0:nlam-olam, gap_spec, ss2.spec[olam:*] ]
...
;%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      if gapsize le 0 then begin
              ...
              ;  MG-- ADDED CASE WHERE WAVELENGTHS OVERLAP
              ;       OVERLAP REGION IS A WEIGHTED AVERAGE
                 gapsize = -1 * gapsize
                 lambda1 = ss1.lambda[0:nlam-gapsize-1]
                 lambda2 = ss2.lambda[gapsize+1:*]

                 gap_lam = ss1.lambda[nlam-gapsize:nlam-1]
                 gap_ivar = fltarr(gapsize) + 1E-20

                 blu_lev = ss1.spec[nlam-gapsize:nlam-1]
                 blu_ivar = ss1.ivar[nlam-gapsize:nlam-1]
                 red_lam = ss2.lambda[0:gapsize]
                 x_specrebin, red_lam, ss2.spec[0:gapsize],gap_lam,red_lev
                 x_specrebin, red_lam, ss2.ivar[0:gapsize],gap_lam,red_ivar

                 tim = blu_lev*blu_ivar + red_lev*red_ivar
                 bim = blu_ivar + red_ivar

                 gap_spec = tim/bim
                 gap_ivar = bim

                spec = [ss1.spec[0:nlam-gapsize-1], gap_spec, ss2.spec[gapsize+1:*] ]
                ivar = [ss1.ivar[0:nlam-gapsize-1], gap_ivar, ss2.ivar[gapsize+1:*] ]
                lambda = [lambda1, gap_lam, lambda2]

;                plot,ss2.lambda,ss2.spec,xrange=[7700,7780],xstyle=1
;                oplot,ss1.lambda,ss1.spec,color=180
;                oplot,lambda,spec,color=100
;                stop

                ss1d = make_ss1d_ovl(spec=spec, ivar=ivar, lambda=lambda, $
                       ssBd=ss1, ssRd=ss2, gapsz=gapsize) 
                goto, ovlskip3
